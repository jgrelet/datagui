...
% --------------------------------------------------------------
FileTsgPos = strcat('../../COLCOR/ascii', filesep, cruise, '_tsg');
...
% ---------------------------------------------------------
latmin = -22; latmax = -19; latint = 1;
lonmin = 53; lonmax = 56; lonint = 1;

% Contours de bathymétrie
% -----------------------
niveaux = [-3000 -2000 -1000];
...
lon = char(lon);
lon = HMtoH(str2num(lon(:,(1:2))) +  str2num(lon(:,(4:8))) /100);
ind =find(char(lons) == 'W');
for i=1:size(ind)
  lon(ind(i)) = lon(ind(i)) * -1;
end 
