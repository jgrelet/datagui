...
#
# J Grelet IRD march 2016 LAPEROUSE cruise
#
# BUGS: 
# avec fichiers cnv, enregistre la montée et ne lit pas les fichiers .xmlcon
# avec la version 5.22.1 sous cygwin, msg: perl Redundant argument in printf
# Si TYPE non present dans header seabird, affichage incorrect de pmax
#
# TODOS:
# Afficher date/heure de  fin de cats dans fichier .ctd 
# 
# $I$ 
...
sub usage() {
  ...
  print STDERR   "example:\n\$ perl ctd-all.pl --cycle_mesure=$cycle_mesure --institute=$institute --plateforme='$plateforme' --sn=$ctdSn --type=$ctdType --pi=$pi --begin_date=$begin_date --end_date=$end_date --echo  data/asc/$cruisePrefix*.hdr\n"; 
  exit 1;
}
...
sub entete_odv { 
  ...
  print  ODV_FILE "//\n"; 
  print  ODV_FILE "Cruise\tStation\tType\tyyyy-mm-ddThh:mm\tLongitude [degrees_east]\tLatitude [degrees_north]\tBot. Depth [m]";
...
for( my $i = 0; $i <= $#ARGV; $i++ ){
  ...
  for my $file (@files_descr) {
     printf $file "%05d  %d %7.3f %7.4f %8.4f %s %6.4g", 
  ...
  while( <DATA_FILE> ){           
	...
	my $nbin = $data{'NBIN'};
	  ...
	  printf ODV_FILE ($bottom_depth > 1e35) ? ("%c", $dummy) : ("\t%6.1f", $bottom_depth);
