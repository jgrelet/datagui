...
sub usage() {
  ...
  print STDERR   "example:\n\$ perl ctd-all.pl --cycle_mesure=$cycle_mesure --institut=$institut --plateforme='$plateforme' --sn=$ctdSn --type=$ctdType --pi=$pi --date_debut=$date_debut --date_fin=$date_fin --echo  --xml data/asc/$cruisePrefix?????.hdr\n"; 
  exit 1;
}
...
sub write_xml_header { 
  ...
  print  XML_CTD_FILE "    </PLATEFORME>\n";
  print  XML_CTD_FILE "    <CYCLE_MESURE CONTEXTE=\"$CONTEXTE\" TIMEZONE=\"$TIMEZONE\" FORMAT=\"$FORMAT_DATE\">\n";  
  print  XML_CTD_FILE "      <LIBELLE>$cycle_mesure</LIBELLE>\n";  
  print  XML_CTD_FILE "      <DATE_DEBUT>$date_debut</DATE_DEBUT>\n";  
  print  XML_CTD_FILE "      <DATE_FIN>$date_fin</DATE_FIN>\n";  
  print  XML_CTD_FILE "      <INSTITUT>$institut</INSTITUT>\n";  
  ...
  print  XML_CTD_FILE "        <CODE>$CODE</CODE>\n";	    
...
if( $DEBUG ) {
  ...
  printf "%s\n%s\n%s\n%s\n%s\n",
          $institut,$plateforme,$ctdType,$ctdSn,$pi;
  for( my $i = 0; $i <= $#ARGV; $i++ ){
    print $ARGV[$i] . "\n";
  }
