function read_json_file(self, varargin)
% read_json_file: read dynaload descriptor from JSON file
%   it's a modified version of read_json_file which is part of jsonlab toolbox
%
...
%            date: 2008/07/03
%
% $Id: read_json_file.m 226 2013-01-29 16:54:48Z jgrelet $
%
% input:
...
% begining of loadjson
% --------------------
if(regexp(fname,'[\{\}\]\[]','once'))
  string=fname;
elseif(exist(fname,'file'))
  fid = fopen(fname,'rt');
  % get all content file in memory
  string = fscanf(fid,'%c');
  fclose(fid);
else
...
% data contain json structure, loop over fielnames
% ------------------------------------------------
dp = fieldnames(data);
for i = 1: length(dp)
  
 % create dynamic properties and initialize it to empty structure
  % ---------------------------------------------------------------
  prop = dp{i};
