...
%
%     >> t = datagui.tests.hashtable('testKeys');
%     >> t.run
...
classdef hashtable < TestCase
  ...
  methods
    ...
    function testKeys( self )
      h = datagui.hashtable( self.key, self.value);
      assertEqual(keys(h), self.key, 'list of keys don''t match');
      h = datagui.hashtable( self.doubleKey, self.doubleValue);
    ...
    function testHashtableConstructorMatch(self)
      assertExceptionThrown( @() datagui.hashtable('a', 'b', 'c'), ...
    ...
    function testVector(self)
      h = datagui.hashtable( 'a', self.vector);
    ...
    function testArray(self)
      h = datagui.hashtable( 'a', self.array);
