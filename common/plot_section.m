...
% =========================================================================
% $Id: plot_section.m 162 2011-09-02 12:36:13Z jgrelet $
%
% Usage:
...
% get root, stockée dans le champ UserData de l'élément racine 0
% --------------------------------------------------------------
root = get(0, 'UserData' );

% Initialise l'instance de la classe profil
% -----------------------------------------
self = root.self;
r = roscop(self);

profils = get(data_1d(self),'PROFILS');
varx = get(data_1d(self), var_x);
var  = get(data_2d(self), code_roscop);
vary = get(data_2d(self), 'PRES');
yr = get(r, 'PRES');  % pour y roscop
y_cle = 'PRES';
...
xr = get(r, var_x);        % pour x roscop LATX, LONX ou DAYD
...
if (~exist( 'liste_profils','var') == 1)
  liste_profils = profils; % trace tous les profils par défaut
end
...
xlabel(H(end), [var_x ': ' xr.description  ' (' xr.unit ')']);
...
switch profilSpec
  case 'on'
    ...
    % ---------------------------------------------------
    if varx(find(profils == profil(1))) < varx(find(profils == profil(2)))
